      ...
      <div className="max-w-7xl mx-auto px-6 text-left">
            ...
            {reviews.map((review) => (
              <div key={review.id} className="bg-white p-6 rounded-2xl shadow-md border hover:shadow-xl transition group">
                <div className="flex items-center mb-4">
                  {/* ROUND PROFILE IMAGE */}
                  <div className="w-14 h-14 rounded-full overflow-hidden mr-4 border-2 border-yellow-200 bg-gray-100 flex items-center justify-center">
                    <img 
                  ...
                  <div className="flex-1">
                    <h4 className="font-bold text-gray-800">
                      {review.instagram_username ? `@${review.instagram_username}` : review.email.split('@')[0]}
                    </h4>
                    <div className="flex text-yellow-400">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-4 h-4 ${i < review.rating ? "fill-yellow-400" : "text-gray-200"}`} />
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                      <button onClick={() => startEdit(review)} className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-full"><Edit className="w-4 h-4" /></button>
                      <button onClick={() => handleDelete(review.id)} className="p-1.5 text-red-500 hover:bg-red-100 rounded-full"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                </div>
        ...
        {!isFormOpen && (
          <div className="flex justify-center mb-8">
            <button onClick={() => setIsFormOpen(true)} className="bg-yellow-500 text-white font-bold py-4 px-10 rounded-full shadow-lg hover:scale-105 transition">
              Write a Review
            </button>
          </div>
        )}
        ...
        {/* --- 3. Form Box --- */}
        {isFormOpen && (
          <div className={`w-full p-8 rounded-3xl shadow-2xl border-4 ${editingId ? 'bg-blue-50 border-blue-400' : 'bg-white border-yellow-400'}`}>
            <div className="max-w-3xl mx-auto">
              <div className="flex justify-between items-center mb-8 border-b pb-4">
                <h3 className="text-2xl font-bold">{editingId ? "Edit Your Review" : "Share Your Experience"}</h3>
                <button onClick={() => { setIsFormOpen(false); cancelEdit(); }} className="p-2 bg-gray-100 rounded-full"><X className="w-6 h-6 text-gray-500" /></button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-2">Instagram Username (Optional)</label>
                    <input
                      type="text"
                      className="w-full px-4 py-3 rounded-xl border-2 outline-none focus:border-yellow-400 transition"
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border-2 border-dashed border-gray-200">
                  <label className="block text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                    <Camera className="w-4 h-4" /> Upload Profile Photo (Optional)
                  </label>
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  />
                  <p className="mt-2 text-xs text-gray-400">Supported formats: JPG, PNG, GIF</p>
                </div>
                ...
                <div>
                  ...
                  <div className="flex space-x-2">
                    {[1, 2, 3, 4, 5].map((s) => (
                      <Star key={s} className={`w-10 h-10 cursor-pointer ${s <= (hover || formData.rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-200"}`} onClick={() => setFormData({ ...formData, rating: s })} onMouseEnter={() => setHover(s)} onMouseLeave={() => setHover(0)} />
                    ))}
                  </div>
                </div>
                ...
                <button type="submit" disabled={submitting} className="w-full bg-yellow-500 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-yellow-600 transition">
                  {submitting ? <Loader2 className="animate-spin mx-auto w-6 h-6" /> : <span>{editingId ? "Update Now" : "Post Review"}</span>}
                </button>
              </form>
            </div>
          </div>
